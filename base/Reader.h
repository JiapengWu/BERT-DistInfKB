...
extern "C"
void print_entity_vocab(){
	for (INT i = 0; i < trainTotal; i++) {
		printf("%ld ", trainList[i].h);
		printf("%ld ", trainList[i].h);
		printf("%ld ", trainList[i].h);
		printf("%s", "\n");
	}
...
void importTrainFiles() {
	...
	// sampling only takes data from triple2id.txt, not entity2id nor others
	for (INT i = 0; i < trainTotal; i++) {
...
void importTestFiles() {
    ...
    FILE* f_kb1 = fopen((inPath + "triple2id.txt").c_str(), "r");
    FILE* f_kb2 = fopen((inPath + "triple2id.txt").c_str(), "r");
    FILE* f_kb3 = fopen((inPath + "triple2id.txt").c_str(), "r");
    ...
    printf("Done with base/reader.h\n");
}
